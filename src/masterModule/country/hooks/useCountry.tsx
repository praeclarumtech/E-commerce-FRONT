...
import { deleteCountry, getCountry } from "../api";
...
function useCountry() {
    ...
    const { data, isLoading, isFetching, isRefetching, error } = useQuery({
        ...
        queryFn: () => getCountry({ params: { page: params.page, limit: params.limit, search: params.search } }),
    ...
    const handleEdit = useCallback((countryName: Country) => {
        navigate(`/country/edit/${countryName._id}`);
    }, [navigate]);

    const handleDeleteClick = useCallback((countryName: Country) => {
    ...
    const handleDeleteConfirm = useCallback(() => {
        if (countryToDelete) {
            deleteMutate(countryToDelete._id);
        }
    }, [countryToDelete, deleteMutate]);
    ...
    const columns: ColumnDef<Country>[] = useMemo(() => {
        const _columns: ColumnDef<Country>[] = [
            {
                header: 'Sr No.',
